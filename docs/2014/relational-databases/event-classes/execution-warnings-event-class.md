...
|Durata|`bigint`|Tempo, in millisecondi, di attesa della query per ottenere la memoria necessaria. Valore valido solo quando EventSubClass = 1 (attesa della query).|13|Yes|  
|Errore|`int`|Non usato.|31|Yes|  
|EventClass|`int`|Tipo di evento = 67.|27|No|  
|EventSequence|`int`|Sequenza di un evento specifico nella traccia.|51|No|  
|EventSubClass|`int`|Tipo di sottoclasse di evento.<br /><br /> 1=attesa query<br /><br /> 2=timeout query|21|Yes|  
