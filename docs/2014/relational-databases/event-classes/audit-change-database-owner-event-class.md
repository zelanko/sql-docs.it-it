...
# <a name="audit-change-database-owner-event-class"></a>Audit Change Database Owner - classe di evento
  La classe di evento **Audit Change Database Owner** viene generata quando si utilizza l'istruzione ALTER AUTHORIZATION per modificare il proprietario di un database e vengono controllate le autorizzazioni necessarie a tale scopo.  
...
|**TargetLoginSid**|**immagine**|Per le azioni relative a un account di accesso, il numero di identificazione di sicurezza (SID) dell'account di accesso specifico.|43|Sì|  
