...
|IntegerData|`int`|Numero di righe restituite dall'istruzione.|25|Yes|  
...
|NestLevel|`int`|Livello di nidificazione della stored procedure se l'istruzione è stata eseguita all'interno di una stored procedure.|29|Yes|  
...
|Reads|`bigint`|Numero di letture di pagine eseguite dall'istruzione SQL.|16|Yes|  
...
|TextData|`ntext`|Testo dell'istruzione eseguita.|1|Yes|  
|TransactionID|`bigint`|ID della transazione se l'istruzione è stata eseguita all'interno di una transazione.|4|Yes|  
|Writes|`bigint`|Numero di scritture di pagina eseguite dall'istruzione SQL.|17|Yes|  
