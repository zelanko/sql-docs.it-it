...
# <a name="audit-database-scope-gdr-event-class"></a>Audit Database Scope GDR - classe di evento
  La classe di evento **Audit Database Scope GDR** viene generata ogni volta che viene eseguita un'istruzione Grant, REVOKE o Deny per un'autorizzazione per le [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] istruzioni da parte di qualsiasi utente in per azioni solo del database, ad esempio la concessione di autorizzazioni per un database.  
