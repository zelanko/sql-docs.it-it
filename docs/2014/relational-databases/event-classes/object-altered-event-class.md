...
|IntegerData|`int`|Numero di sequenza dell'evento corrispondente della fase di inizio. Questa colonna è disponibile solo per la sottoclasse di evento di tipo Commit o Rollback. Sottoclasse.|25|Yes|  
|IsSystem|`int`|Indica se l'evento è stato generato per un processo di sistema o un processo utente. 1 = sistema, NULL = utente.|60|Yes|  
...
|ObjectID2|`bigint`|ID della funzione di partizione in caso di modifica dello schema di partizione, ID della coda in caso di modifica del servizio o ID dello schema di raccolta in caso di modifica dell'XML Schema.|56|Yes|  
...
|RequestID|`int`|ID della richiesta di batch contenente l'istruzione.|49|Yes|  
