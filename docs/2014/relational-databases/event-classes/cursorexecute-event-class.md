...
# <a name="cursorexecute-event-class"></a>CursorExecute - classe di evento
  La classe di evento **CursorExecute** descrive gli eventi di esecuzione del cursore che si verificano nei cursori API (Application Programming Interface). Gli eventi di esecuzione del [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] cursore vengono generati quando crea e popola un cursore dal piano di esecuzione creato da un evento di preparazione del cursore.  
...
|**Gestire**|**int**|Valore integer utilizzato da ODBC, OLE DB o DB-Library per il coordinamento dell'esecuzione con il server.|33|Sì|  
