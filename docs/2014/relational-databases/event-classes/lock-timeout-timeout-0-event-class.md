...
|DatabaseID|`int`|ID del database in cui si è verificato il timeout. Se la colonna di dati [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] viene catturata nella traccia e il server è disponibile `ServerName` visualizza il nome del database. Determinare il valore per un database utilizzando la funzione DB_ID.|3|Sì|  
