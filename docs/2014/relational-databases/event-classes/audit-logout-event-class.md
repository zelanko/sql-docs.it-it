...
# <a name="audit-logout-event-class"></a>Audit Logout - classe di evento
  La classe di evento **Audit Logout** indica che un utente ha effettuato la disconnessione da ( [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]disconnesso). Gli eventi di questa classe vengono attivati da nuove connessioni o da connessioni riutilizzate da un pool di connessioni.  
...
|**Letture**|**bigint**|Numero di operazioni di I/O di lettura logica eseguite dall'utente durante la connessione.|16|Sì|  
...
|**Scritture**|**bigint**|Numero di operazioni di I/O di scrittura logica eseguite dall'utente durante la connessione.|17|Sì|  
