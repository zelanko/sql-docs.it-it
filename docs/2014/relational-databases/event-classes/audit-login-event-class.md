...
# <a name="audit-login-event-class"></a>Audit Login - classe di evento
  La classe di evento **Audit Login** indica che un utente ha eseguito l'accesso [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]a. Gli eventi di questa classe vengono attivati da nuove connessioni o da connessioni riutilizzate da un pool di connessioni.  
...
|**BinaryData**|**immagine**|Impostazioni del livello di sessione, inclusi i valori NULL ANSI, il riempimento con caratteri ANSI, la chiusura del cursore in fase di commit, la concatenazione dei valori NULL e gli identificatori tra virgolette.|2|Sì|  
