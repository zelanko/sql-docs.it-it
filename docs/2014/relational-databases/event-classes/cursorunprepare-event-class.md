...
ms.locfileid: "62663395"
---
# <a name="cursorunprepare-event-class"></a>CursorUnprepare - classe di evento
  La classe di evento **CursorUnprepare** fornisce informazioni sugli eventi di non preparazione del cursore che si verificano nei cursori API (Application Programming Interface). Gli eventi di annullamento della preparazione del [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] cursore si verificano quando elimina un piano di esecuzione.  
...
|**DatabaseID**|**int**|ID del database specificato nell'istruzione USE oppure ID del database predefinito, se per una determinata istanza non viene eseguita un'istruzione USE. [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)]consente di visualizzare il nome del database se la colonna di dati **ServerName** viene acquisita nella traccia e il server è disponibile. Determinare il valore per un database utilizzando la funzione DB_ID.|3|Sì|  
...
|**Gestire**|**Int**|Identifica l'handle preparato che sta per essere convertito in non preparato.|33|Sì|  
