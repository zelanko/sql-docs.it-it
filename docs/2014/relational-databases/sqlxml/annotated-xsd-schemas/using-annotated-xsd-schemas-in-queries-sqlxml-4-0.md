...
```  
 ...
 Il modello specifica inoltre due query XPath. Ognuno degli elementi ** \<>di query XPath** identifica in modo univoco lo schema di mapping specificando l' `mapping-schema` attributo.  
  
 Quando si specifica uno schema inline nel modello, è `sql:is-mapping-schema` necessario specificare anche l'annotazione nell'elemento ** \< xsd: schema>** . `sql:is-mapping-schema` utilizza un valore booleano (0=false, 1=true). Uno schema inline con **SQL: is-mapping-schema = "1"** viene considerato come schema con annotazioni inline e non viene restituito nel documento XML.  
...
-   Specificare l'attributo `mapping schema` utilizzando l'oggetto ADO `Command`.  
 ...
 L' `mapping-schema` attributo specificato nell'elemento ** \< xpath-query>** o ** \< attributo updg: Sync>** ha la precedenza più elevata; l' `Command` oggetto ADO ha la precedenza più bassa.  
