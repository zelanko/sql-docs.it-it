...
|[model](../databases/model-database.md)|Modello per tutti i database creati nell'istanza di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].|Yes|Configurabile dall'utente<sup>1</sup>|Eseguire il backup di **model** solo se necessario in base alle esigenze aziendali, ad esempio immediatamente dopo la personalizzazione delle opzioni del database.<br /><br /> **Procedura consigliata:** Si consiglia di creare solo backup completi del database **modello**come richiesto. Poiché nel database **model** vengono apportate solo di rado lievi modifiche, il backup del log non è necessario.|  
