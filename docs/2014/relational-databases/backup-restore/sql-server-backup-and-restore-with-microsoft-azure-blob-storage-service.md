...
## <a name="benefits-of-using-the-azure-blob-service-for-includessnoversionincludesssnoversion-mdmd-backups"></a>Vantaggi dell'uso del servizio BLOB di Azure per i backup di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
...
-   Archivio di backup: il servizio di archiviazione BLOB di Azure offre un'alternativa migliore rispetto all'opzione Tape spesso usata per archiviare i backup. Per l'archiviazione su nastro potrebbero essere necessari il trasporto fisico in una struttura esterna e misure di protezione dei supporti. L'archiviazione dei backup nell'archiviazione BLOB di Azure offre un'opzione di archiviazione immediata, a disponibilità elevata e durevole.  
  
-   Nessun overhead di gestione dell'hardware: non è previsto alcun sovraccarico di gestione hardware con i servizi di Azure. I servizi di Azure gestiscono l'hardware e forniscono la replica geografica per la ridondanza e la protezione da errori hardware.  
...
-   Attualmente, per le istanze di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in esecuzione in una macchina virtuale di Azure, il backup nei servizi di archiviazione BLOB di Azure può essere eseguito creando dischi collegati. Tuttavia, esiste un limite al numero di dischi che è possibile connettersi a una macchina virtuale di Azure. vale a dire 16 per un'istanza di dimensioni elevate e un po' di meno per istanze di dimensioni inferiori. Abilitando un backup diretto nell'archiviazione BLOB di Azure, è possibile ignorare il limite di 16 dischi.  
     ...
     Inoltre, il file di backup che ora è archiviato nel servizio di archiviazione BLOB di Azure è direttamente disponibile in un [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] locale o in un'altra [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in esecuzione in una macchina virtuale di Azure, senza necessità di collegamento/scollegamento del database o di download e collegamento del disco rigido virtuale.  
  
-   Vantaggi economici: si paga solo per il servizio usato. Questo servizio può essere efficace dal punto di vista economico come soluzione di archiviazione esterna e di backup. Per ulteriori informazioni e collegamenti, vedere la sezione [considerazioni sulla fatturazione di Azure](#Billing) .  
 ...
 **Archiviazione:** i costi dipendono dallo spazio usato e vengono calcolati in base a una scala graduata e a livello di ridondanza. Per ulteriori dettagli e informazioni aggiornate, vedere la sezione **Gestione dati** dell'articolo [Dettagli prezzi](https://go.microsoft.com/fwlink/?LinkId=277059) .  
