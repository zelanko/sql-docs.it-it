...
4.  Completare un'operazione dalla tabella seguente:  
    ...
    |Impostare l'opzione cluster|Nella griglia sotto **Progettazione tabelle**selezionare **Crea come CLUSTERED** e dall'elenco a discesa scegliere Sì per creare un indice cluster e No per crearne uno non cluster. Per ogni tabella è possibile creare un solo indice cluster. Se esiste già un indice cluster in questa tabella, deselezionare questa opzione sull'indice originale.|  
