...
|*MachineName*$@*Dominio* NomeComputer|Specifica diretta di un account del computer.<br /><br /> Se il server a cui ci si connette è in esecuzione con `ServerSPN` account LOCAL SYSTEM o NETWORK SERVICE, per ottenere l'autenticazione Kerberos, può essere nel formato *MachineName*$@*Domain.*|  
