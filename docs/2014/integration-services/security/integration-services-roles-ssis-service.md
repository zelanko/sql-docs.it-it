...
## <a name="permissions"></a>Autorizzazioni  
 Per impostazione predefinita, le autorizzazioni del `db_ssisadmin` e **db_ssisoperator** i ruoli predefiniti a livello di database e l'ID di sicurezza univoco dell'utente che ha creato il pacchetto si applicano al ruolo lettore per i pacchetti, `db_ssisadmin` mentre le autorizzazioni del ruolo e l'ID di sicurezza univoco dell'utente che ha creato il pacchetto si applicano al ruolo writer. Per disporre dell'accesso in lettura al pacchetto `db_ssisadmin`, un utente deve essere un membro del ruolo, **db_ssisltduser**o **db_ssisoperator** . Un utente deve essere membro del `db_ssisadmin` ruolo per avere accesso in scrittura.  
...
## <a name="access-to-packages"></a>Accesso ai pacchetti  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]per potersi connettere al motore di database e accedere al `msdb` database, è necessario avviare il servizio MSSQLSERVER.  
