...
> [!NOTE]  
>  La destinazione ADO NET non supporta l'esecuzione del mapping di una colonna di input il cui tipo è impostato su DT_DBTIME su una colonna del database il cui tipo è impostato su datetime. Per ulteriori informazioni sui [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] tipi di dati, vedere [Integration Services tipi di dati](integration-services-data-types.md).  
