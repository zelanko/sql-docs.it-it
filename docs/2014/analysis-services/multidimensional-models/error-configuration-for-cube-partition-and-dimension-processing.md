...
-   Impostare `NullProcessing` = `Error` per escludere i record con valori null. Verrà generato l'errore `NullKeyNotAllowed`, che viene registrato e conteggiato per il limite di errori di chiave. È possibile impostare proprietà di configurazione errori sul **chiave Null non consentita** a `IgnoreError` per continuare l'elaborazione.  
