...
1.  **Origini dati nel riquadro dei dati del report** Un'origine dati viene visualizzata nel riquadro dei dati del report dopo la creazione di un'origine dati incorporata o l'aggiunta di un'origine dati condivisa.  
  
2.  Finestra di **dialogo connessione** Utilizzare la finestra di dialogo connessione per compilare una stringa di connessione o per incollare una stringa di connessione.  
  
3.  **Informazioni sulla connessione dati** La stringa di connessione viene passata all'estensione per i dati.  
  
4.  **Credenziali** di Le credenziali vengono gestite separatamente dalla stringa di connessione.  
  
5.  **Estensione per i dati/provider di dati** La connessione ai dati può avvenire attraverso più livelli di accesso ai dati.  
  
6.  **Origini dati esterne** Recuperare dati da database relazionali, basi dati multidimensionali, elenchi SharePoint, servizi Web o modelli di report.  
...
|[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]database dell'istanza|`Data Source=localhost\MSSQL12.InstanceName; Initial Catalog= AdventureWorks2012`|Impostare il tipo di origine dati su `SQL Server`.|  
