...
ms.locfileid: "78175040"
---
# <a name="reporting-services-in-sql-server-data-tools-ssdt"></a>Reporting Services in SQL Server Data Tools (SSDT)
  [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)]è un [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)] ambiente con miglioramenti specifici per le soluzioni Business Intelligence. 
  [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)] è incluso in [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].

 Usare [!INCLUDE[ssBIDevStudio](../../../includes/ssbidevstudio-md.md)] per la creazione e la gestione di soluzioni e progetti per report di [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] ed elementi correlati ai report. 
  [!INCLUDE[ssBIDevStudio](../../../includes/ssbidevstudio-md.md)] fornisce l'ambiente di creazione di Progettazione report. In Progettazione report è possibile aprire, modificare, visualizzare in anteprima, salvare e distribuire definizioni di report, origini dati condivise, set di dati condivisi e parti di report.
...
##  <a name="bkmk_ReportDesignerWindowsandPanes"></a>Progettazione report finestre e riquadri
...
###  <a name="bkmk_ReportDataPane"></a>Riquadro dei dati del report
...
-   **Campi predefiniti** Informazioni predefinite sul report, ad esempio il nome o l'ora di elaborazione del report.

-   **Origini dati** Un'origine dati rappresenta un nome e una connessione a un'origine dati.

-   **Set di impostazioni** Ogni set di dati include una query che specifica i dati da recuperare dall'origine dati. Espandere il set di dati per visualizzare la raccolta di campi specificati dalla query del set di dati.
...
-   **Parametri del report** Elenco di parametri del report. Se una query del set di dati include parametri di query, è possibile creare i parametri manualmente o automaticamente.

-   **Immagini** di Elenco di immagini disponibili da includere come elemento del report immagine in un report.
...
###  <a name="bkmk_PropertiesPane"></a>Riquadro proprietà
...
##  <a name="bkmk_ReportDesignerDesignView"></a>Visualizzazione Progettazione Progettazione report
...
##  <a name="bkmk_ReportDesignerPreview"></a>Anteprima Progettazione report
...
-   **Colore primo piano** Selezionare un colore del testo. Il colore predefinito è il nero.

-   **Colore di sfondo** Selezionare un colore di sfondo per le caselle di testo e le aree dati.

-   **Carattere** Consente di specificare se il testo è in grassetto, corsivo o sottolineato.

-   **Giustifica** Consente di specificare se il testo è allineato a destra, centrato o allineato a sinistra.

-   **Allinea** a Consente di specificare il modo in cui gli oggetti selezionati sono allineati in relazione tra loro all'interno del report.

-   **Rendi uguali** Consente di regolare le dimensioni degli oggetti selezionati all'interno del report.

-   **Spaziatura orizzontale** Consente di regolare la spaziatura orizzontale tra gli oggetti selezionati all'interno del report.

-   **Spaziatura verticale** Consente di regolare la spaziatura verticale tra gli oggetti selezionati all'interno del report.

-   **Centra nel form** Centrare l'oggetto selezionato verticalmente e orizzontalmente in relazione alla finestra Progettazione report.

-   **Ordine** di Spostare gli oggetti selezionati in background o in primo piano.
...
-   **Proprietà report** Selezionare per aprire la finestra di dialogo **Proprietà rapporto** . in cui è possibile assegnare proprietà generali del report come il nome dell'autore e la spaziatura della griglia, nonché proprietà per il layout come il numero di colonne e le dimensioni pagina. È inoltre possibile includere codice personalizzato, riferimenti ad assembly e classi e i nomi di elementi di output dei dati, trasformazioni dati e schemi dati.

-   **Visualizza** Passare tra le due schede Progettazione report: progettazione e anteprima.

-   **Intestazione pagina** Aggiungere o eliminare un'intestazione di pagina nel report. L'eliminazione di un'intestazione di pagina implica la rimozione di tutti gli elementi al suo interno.

-   **Piè di pagina** Aggiungere o eliminare un piè di pagina nel report. L'eliminazione di un piè di pagina implica la rimozione di tutti gli elementi al suo interno.

-   **Riquadro di raggruppamento** Mostrare o nascondere il riquadro di raggruppamento.
...
-   **Altre finestre** Usare questa opzione per visualizzare il riquadro seguente:
    ...
    -   **Struttura documento** Utilizzare questa opzione per visualizzare una visualizzazione gerarchica degli elementi del report e le relative raccolte di caselle di testo in un report.

-   **Barre degli strumenti** Utilizzare questa opzione per visualizzare le barre degli strumenti che supportano le funzionalità di Progettazione report, inclusi i **bordi del report** e la **formattazione del report**. Per altre informazioni, vedere [Barre degli strumenti di Progettazione report](#bkmk_ReportDesignerToolbars).

-   **Dati del report** Utilizzare questa opzione per visualizzare il riquadro dei dati del report, in cui è possibile aggiungere parametri, origini dati, set di dati e immagini del report.
...
-   **Aggiungi elemento esistente** Aggiungere un'origine dati condivisa esistente o un report esistente al progetto.

-   **Importa report** Importare report da un'altra applicazione, ad esempio Microsoft Access.

-   **Escludi dal progetto** Esclude gli elementi dal progetto. Con questa opzione l'elemento non viene eliminato dal file system.

-   **Mostra tutti i file** Mostra tutti i file in un progetto.

-   **Aggiorna elementi della casella degli strumenti del progetto** Aggiornare la cache della casella degli strumenti quando si installano nuovi elementi del report personalizzati nel progetto.

-   **Proprietà** di Aprire la finestra di dialogo **pagine delle proprietà** per il progetto. Per altre informazioni, vedere [Finestra di dialogo Pagine delle proprietà del progetto](project-property-pages-dialog-box.md).

##  <a name="bkmk_ReportDesignerToolbars"></a>Barre degli strumenti Progettazione report
...
-   **Report** di Aggiungere un'intestazione o un piè di pagina, impostare le proprietà del report, abilitare o disabilitare il righello o il riquadro di raggruppamento oppure utilizzare zoom per modificare la visualizzazione del report.

-   **Bordi del report** Imposta il colore, lo stile e la larghezza per tutte le righe selezionate e i bordi di tutti gli elementi del report selezionati.

-   **Formattazione del report** Imposta il formato degli elementi del report selezionati. Per le caselle di testo, è possibile modificare i tipi di formattazione seguenti tramite la barra degli strumenti: proprietà del carattere e colore del testo, colore di sfondo e giustificazione del testo.

-   **Layout** di Imposta l'ordine di disegno degli elementi del report e l'Unione di celle in un'area dati.

-   **Standard** Aprire o salvare progetti, visualizzare finestre e selezionare la configurazione di debug.
...
-   **/deploy** Distribuire i report usando le proprietà del progetto specificate in un file di configurazione. Ad esempio, nel comando seguente vengono distribuiti i report specificati dal file di soluzione Reports.sln tramite le impostazioni di configurazione Release specificate nelle proprietà del progetto:

    ```
    devenv.exe "C:\Users\MyUser\Documents\Visual Studio 2010\Projects\Reports\Reports.sln" /deploy "Release"
    ```

-   **/Build** Compilare il file della soluzione senza distribuirlo. Ad esempio, nel comando seguente vengono compilati i report specificati dal file di soluzione Reports.sln tramite le impostazioni di configurazione Debug specificate nelle proprietà del progetto:
...
##  <a name="bkmk_KeyboardShortcuts"></a>Tasti di scelta rapida in Reporting Services
