---
title: Aggiornamento di PowerPivot per SharePoint . Documenti Microsoft
ms.custom: ''
ms.date: 03/25/2019
...
-   Se l'installazione di [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)]PowerPivot esistente è , il [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)] Service Pack [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]2 (SP2) è necessario per un aggiornamento a .  
  
-   Se l'installazione di [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]PowerPivot esistente è , il [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] Service Pack [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]1 (SP1) è necessario per un aggiornamento a .  
...
1.  Eseguire il programma di installazione di [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] nei server back-end che eseguono [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] in modalità SharePoint. Se il server ospita più istanze di [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)], aggiornare almeno l'istanza di **POWERPIVOT** . Nell'elenco seguente vengono riepilogati i passaggi dell'Installazione guidata correlati a un aggiornamento di [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] :  
    ...
    4.  Per altre informazioni, vedere [Eseguire l'aggiornamento a SQL Server 2014 Tramite l'Installazione guidata &#40;&#41;](../../database-engine/install-windows/upgrade-sql-server-using-the-installation-wizard-setup.md)  
...
4.  Eseguire lo strumento di configurazione **PowerPivot per SharePoint 2013** in uno dei server applicazioni di SharePoint per configurare la farm di SharePoint con i file di soluzione aggiornati installati dal componente aggiuntivo. Non è possibile utilizzare Amministrazione centrale SharePoint per questo passaggio. Per altre informazioni, vedere gli argomenti seguenti:  
  
    1.  Nella pagina Start di Windows digitare **PowerPivot** e nei risultati della ricerca fare clic su **Configurazione PowerPivot per SharePoint 2013**. Si noti che la ricerca può restituire entrambe le versioni dello strumento di configurazione.  
         ...
         Fare clic sul **pulsante Start** [!INCLUDE[ssCurrentUI](../../includes/sscurrentui-md.md)], scegliere Tutti i **programmi**, fare clic su Strumenti di **configurazione**e quindi su **Configurazione PowerPivot per SharePoint 2013 .** Si noti che lo strumento è elencato solo se [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] è installato nel server locale.  
  
    2.  All'avvio, tramite lo strumento di configurazione viene controllato lo stato dell'aggiornamento della soluzione farm PowerPivot e delle soluzioni applicazione Web PowerPivot. Se vengono rilevate versioni precedenti di queste soluzioni, verrà visualizzato il messaggio "Sono state rilevate versioni più recenti dei file di soluzione PowerPivot.If older versions of these solutions are detected, you will see the message **"Newer versions of the PowerPivot solution files have been detected. Selezionare l'opzione di aggiornamento per aggiornare la farm."** Fare clic su **OK** per chiudere il messaggio di convalida del sistema.  
    ...
    7.  Fare clic su **Esegui** per eseguire tutte le azioni valide per questa attività. L'opzione**Esegui** è disponibile solo dopo il superamento del controllo di convalida. Quando si fa clic su **Esegui**, viene visualizzato il seguente avviso che ricorda che le azioni vengono elaborate in modalità batch:**"Tutte le impostazioni di configurazione contrassegnate come valide nello strumento verranno applicate alla farm di SharePoint. Vuoi continuare?**".  
  
    8.  Fare clic su **Sì** per continuare.  
  
    9. Il completamento dell'aggiornamento delle soluzioni e delle funzionalità nella farm può richiedere diversi minuti. Durante questo periodo, le richieste di connessione per i dati PowerPivot **avranno esito negativo** con errori simili a "**Impossibile aggiornare i dati**" o " Si è verificato un errore durante il tentativo di eseguire**l'azione richiesta. Riprovare**." Al termine dell'aggiornamento, il server diventerà disponibile e questi errori non si verificheranno più.  
...
3.  Verificare che i servizi **SharePoint** **SQL Server Analysis Services** e il servizio di sistema **PowerPivot** di SQL Server siano avviati in Amministrazione centrale SharePoint o utilizzare il comando Di PowerShell seguente: .  
...
5.  **Eseguire [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] ** il programma di installazione nel primo server applicazioni di SharePoint che esegue il servizio Windows **SQL Server Analysis Services (PowerPivot)** per aggiornare l'istanza di POWERPIVOT. Nella pagina Installazione dell'Installazione guidata di SQL Server scegliere l'opzione di aggiornamento. Per ulteriori informazioni, vedere [Eseguire l'aggiornamento a SQL Server 2014 tramite l'Installazione guidata &#40;installazione&#41;](../../database-engine/install-windows/upgrade-sql-server-using-the-installation-wizard-setup.md).  
...
7.  Eseguire lo strumento di **configurazione PowerPivot** nel primo server applicazioni di SharePoint che esegue il servizio SQL Server Analysis Services (PowerPivot) per aggiornare le soluzioni e i servizi Web in SharePoint. Non è possibile utilizzare Amministrazione centrale per questo passaggio.  
  
    1.  Fare clic sul **pulsante Start** [!INCLUDE[ssCurrentUI](../../includes/sscurrentui-md.md)], scegliere Tutti i **programmi**, Fare clic su Strumenti di **configurazione**e quindi su Strumento di **configurazione PowerPivot**. Si noti che lo strumento è elencato solo se [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] è installato nel server locale.  
  
    2.  All'avvio, tramite lo strumento di configurazione viene controllato lo stato dell'aggiornamento della soluzione farm PowerPivot e delle soluzioni applicazione Web PowerPivot. Se vengono rilevate versioni precedenti di queste soluzioni, verrà visualizzato il messaggio "Sono state rilevate versioni più recenti dei file di soluzione PowerPivot.If older versions of these solutions are detected, you will see the message "Newer versions of the PowerPivot solution files have been detected. Selezionare l'opzione di aggiornamento per aggiornare la farm". Fare clic su **OK** per chiudere il messaggio.  
  
    3.  Fare clic su **Aggiorna funzionalità, servizi, applicazioni e soluzioni**, quindi su **OK** per continuare.  
  
    4.  Viene visualizzato il seguente avviso: "Le cartelle di lavoro nel dashboard di gestione PowerPivot stanno per essere aggiornate alla versione più recente. Tutte le personalizzazioni apportate alle cartelle di lavoro esistenti andranno perse. Continuare?"  
         ...
         Per altre informazioni sulla personalizzazione delle cartelle di lavoro usate nel dashboard, vedere Personalizzazione del dashboard di gestione PowerPivot .For more information about customizing workbooks used in the dashboard, see [Customizing the PowerPivot Management Dashboard](https://go.microsoft.com/fwlink/?linkID=229639).  
    ...
    8.  Fare clic su **Esegui** per eseguire tutte le azioni valide per questa attività. L'opzione**Esegui** è disponibile solo dopo il superamento del controllo di convalida. Quando si fa clic su **Esegui**, viene visualizzato il seguente avviso, in cui si ricorda che le azioni vengono elaborate in modalità batch: "Tutte le impostazioni di configurazione contrassegnate come valide nello strumento verranno applicate alla farm di SharePoint. Continuare?"  
  
    9. Fare clic su **Sì** per continuare.  
  
    10. Il completamento dell'aggiornamento delle soluzioni e delle funzionalità nella farm può richiedere diversi minuti. Durante questo periodo, le richieste di connessione per i dati PowerPivot avranno esito negativo con errori come "Impossibile aggiornare i dati" o "Si è verificato un errore durante il tentativo di eseguire l'azione richiesta. Riprovare". Al termine dell'aggiornamento, il server diventerà disponibile e questi errori non si verificheranno più.  
  
8.  **Ripetere il processo** per ogni servizio SQL Server Analysis Services (PowerPivot) nella farm: 1) Eseguire il programma di installazione di SQL Server 2) Eseguire lo strumento di configurazione PowerPivot.Repeat the process for each SQL Server Analysis Services (PowerPivot) service in the farm: 1) Run SQL Server setup 2) Run the PowerPivot Configuration tool.  
...
10. **Risoluzione degli errori**  
    ...
    3.  Per le distribuzioni o le ritrazioni correlate alle soluzioni PowerPivot, copiare il valore GUID per **JobId** e quindi incollarlo nel comando seguente (utilizzare i comandi Mark, Copy e Paste del menu Modifica di Shell per copiare il GUID):  
...
##  <a name="verify-the-versions-of-powerpivot-components-and-services"></a><a name="bkmk_verify_versions"></a>Verificare le versioni dei componenti e dei servizi PowerPivotVerify the Versions of PowerPivot Components and Services  
...
```  
 ...
 Verificare **CurrentSolutionVersion**. [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]è la versione 12.0. \<principali> di costruzione. \<> di compilazione minori  
...
4.  [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]la versione del file deve essere 12.00. \<principali> di costruzione. \<> di compilazione minori.  
 ...
 **12.0.2000.8** 2014.0120.200 C: . PowerPIVOT2000  
...
1.  In Gestione applicazioni in Amministrazione centrale fare clic su Gestisci applicazioni di **servizio.**  
...
3.  Verificare di disporre della versione 12.0.Verify you have version 12.0. \<numero di build>.  
  

##  <a name="upgrading-multiple-powerpivot-for-sharepoint-servers-in-a-sharepoint-farm"></a><a name="geminifarm"></a>Aggiornamento di più server PowerPivot per SharePoint in una farm di SharePointUpgrading Multiple PowerPivot for SharePoint Servers in a SharePoint Farm  
...
##  <a name="applying-a-qfe-to-a-powerpivot-instance-in-the-farm"></a><a name="qfe"></a>Applicazione di un QFE a un'istanza di PowerPivot nella farm  
...
|Nel tempo, monitorare report dell'aggiornamento dati in Dashboard di gestione PowerPivot per verificare che non ci siano errori di aggiornamento dati.|[Dati di utilizzo e dashboard di gestione PowerPivot](https://docs.microsoft.com/analysis-services/power-pivot-sharepoint/power-pivot-management-dashboard-and-usage-data)|  
 ...
 Per istruzioni dettagliate che illustrano tutte le attività di configurazione successive all'installazione, vedere [Configurazione iniziale &#40;PowerPivot per SharePoint&#41;](../../../2014/sql-server/install/initial-configuration-powerpivot-for-sharepoint.md).  
