...
## <a name="catch-and-handle-package-errors-by-using-event-handlers"></a>Intercettazione e gestione degli errori dei pacchetti tramite gestori di eventi  
 Usare i gestori eventi per rispondere agli eventi generati dal pacchetto e dagli oggetti del pacchetto.  
  
-   **Creazione di un gestore dell'evento per l'evento OnError**. Nel gestore dell'evento è possibile usare un'attività di invio messaggi per notificare l'errore a un amministratore. Usare un'attività Script e una logica personalizzata per ottenere informazioni di sistema per la risoluzione dei problemi o per pulire le risorse temporanee e l'output incompleto. Per altre informazioni, vedere [Gestori eventi di Integration Services &#40;SSIS&#41;](../../integration-services/integration-services-ssis-event-handlers.md).  
...
-   **Aggiunta di informazioni descrittive agli output degli errori**. Per rendere più chiaro l'output degli errori, oltre ai due identificatori numerici specificati dall'output stesso aggiungere il messaggio di errore e il nome della colonna. Per un esempio di come aggiungere queste due colonne supplementari mediante scripting, vedere [Ottimizzazione di un output degli errori con il componente script](../../integration-services/extending-packages-scripting-data-flow-script-component-examples/enhancing-an-error-output-with-the-script-component.md).  
...
## <a name="troubleshoot-package-execution-by-using-logging"></a>Risoluzione dei problemi relativi all'esecuzione di pacchetti tramite la registrazione  
 Con la registrazione è possibile tenere traccia di ciò che avviene durante l'esecuzione dei pacchetti. I provider di log consentono di acquisire informazioni sugli eventi specificati da usare per un'analisi successiva e di salvare tali informazioni in uno dei formati supportati.  I formati dei provider di log di supporto includono la tabella di database, un file flat e un file XML.  
...
-   **Configurare la proprietà DelayValidation per elementi del pacchetto non validi noti**. Per evitare errori di convalida per gli elementi del pacchetto con problemi di configurazione noti, impostare **DelayValidation** su **True**. Ad esempio, il pacchetto contiene un'attività Flusso di dati in cui viene usata una tabella di destinazione che non esiste fino a quando non viene creata in fase di esecuzione da un'attività Esegui SQL. La proprietà **DelayValidation** può essere abilitata a livello di pacchetto oppure delle singole attività e dei singoli contenitori del pacchetto.  
...
## <a name="troubleshoot-errors-without-a-description"></a>Risoluzione dei problemi relativi agli errori senza descrizione  
 Se viene generato un errore di [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] senza descrizione, è possibile individuare la descrizione in [Guida di riferimento ai messaggi e agli errori di Integration Services](../../integration-services/integration-services-error-and-message-reference.md). Attualmente nell'elenco non sono incluse informazioni per la risoluzione dei problemi.  
