 ...
 *start_time*  
 ...
 Nel set di risultati vengono incluse solo le righe del capture_instance CDC. < > _CT tabella delle modifiche con un'ora di commit associata maggiore di *start_time* .  
...
-   @closed_high_end_point = 1  
     ...
     Nel set di risultati vengono incluse solo le righe della tabella CDC. < capture_instance > _CT tabella delle modifiche con un'ora di commit associata minore o uguale a end_time.  
...
2.  Eseguire lo script per creare effettivamente la funzione wrapper.  
 ...
 Il set di risultati restituito dalla funzione wrapper **All Changes** restituisce rispettivamente le colonne _ _ $ start_lsn e \_\_$seqval della tabella delle modifiche come colonne \__CDC_STARTLSN e \__CDC_SEQVAL. Segue solo le colonne rilevate visualizzate nella *\@column_list* parametro al momento della generazione del wrapper. Se *\@column_list* è null, vengono restituite tutte le colonne di origine rilevate. Le colonne di origine sono seguite da una colonna operation, \__CDC_OPERATION, ovvero una colonna a uno o due caratteri che identifica l'operazione.  
