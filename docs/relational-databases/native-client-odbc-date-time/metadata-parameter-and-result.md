---
title: Parametri e metadati dei risultati Documenti Microsoft
ms.custom: ''
ms.date: 03/04/2017
ms.prod: sql
...
|SQL_DESC_TYPE_NAME|**Data**|**time**|**smalldatetime** in IRD, **datetime2** in IPD|**datetime** in IRD, **datetime2** in IPD|**datetime2**|datetimeoffset|  
 ...
 **datetime2** viene restituito come typename per **smalldatetime** e **datetime** perché il driver utilizza questo come un tipo comune per la trasmissione di tutti i valori **SQL_TYPE_TIMESTAMP** al server.  
  
 SQL_CA_SS_VARIANT_SQL_TYPE è un nuovo campo di descrizione. Questo campo è stato aggiunto a IRD e IPD per consentire alle applicazioni di specificare il tipo di valore associato a colonne e parametri **sqlvariant** (SQL_SSVARIANT)  
  
 SQL_CA_SS_SERVER_TYPE è un nuovo campo solo IPD che consente alle applicazioni di controllare le modalità di invio al server dei valori per i parametri associati come SQL_TYPE_TYPETIMESTAMP (oppure come SQL_SS_VARIANT con un SQL_C_TYPE_TIMESTAMP di tipo C). Se SQL_DESC_CONCISE_TYPE è SQL_TYPE_TIMESTAMP (o è SQL_SS_VARIANT e il tipo C è SQL_C_TYPE_TIMESTAMP) quando SQLExecute o SQLExecDirect viene chiamato, il valore di SQL_CA_SS_SERVER_TYPE determina il tipo di flusso di dati tabulari (TDS) del valore del parametro, come indicato di seguito:  
...
-   Quando un'applicazione forza una preparazione non posticipata chiamando SQLPrepare con preparazione posticipata disabilitata o chiamando SQLNumResultCols, SQLDescribeCol o SQLDescribeParam per un'istruzione preparata ma non eseguita.  
 ...
 L'attributo SQL_CA_SS_SERVER_TYPE può essere utilizzato da applicazioni che dipendono da funzionalità supportate da **datetime** e **smalldatetime**, ma non **da datetime2**. Ad esempio, **datetime2** richiede l'uso delle funzioni **dateadd** e **datediif,** mentre **datetime** e **smalldatetime** consentono anche operatori aritmetici. Nella maggior parte delle applicazioni è consigliabile evitare l'uso di questo attributo.  
