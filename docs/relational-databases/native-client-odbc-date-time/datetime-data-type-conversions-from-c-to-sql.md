---
title: Conversioni da C a SQL Documenti Microsoft
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  In questo argomento vengono elencati i [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] problemi da considerare quando si esegue la conversione da tipi C a tipi data/ora.  
  
 Le conversioni descritte nella tabella seguente sono valide se effettuate sul client. Nei casi in cui il client specifica la precisione del secondo frazionario per un parametro diverso da quello definito nel server, la conversione del client potrebbe avere esito positivo, ma il server restituirà un errore quando viene chiamato **SQLExecute** o **SQLExecuteDirect.** In particolare, ODBC considera qualsiasi troncamento dei secondi frazionari [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] come un errore, mentre il comportamento è quello di arrotondare; ad esempio, l'arrotondamento si verifica quando si passa da **datetime2(6)** a **datetime2(2)**. I valori della colonna di tipo datetime vengono arrotondati a 1/300° di un secondo e le colonne smalldatetime contengono secondi impostati su zero dal server.  
...
-   **-**: non è supportata alcuna conversione. Viene generato un record di diagnostica con SQLSTATE 07006 e il messaggio "Violazione dell'attributo del tipo di dati".  
...
-   **2**: I campi ora devono essere zero o viene generato un record di diagnostica con SQLSTATE 22008 e il messaggio "Troncamento frazionario".  
...
-   **5**: Il fuso orario è impostato sull'impostazione del fuso orario del client.  
...
-   **10**: Se si verifica il troncamento con la perdita di dati, viene generato un record di diagnostica con SQLSTATE 22008 e il messaggio "Formato ora non valido". Questo errore si verifica anche se il valore non è incluso nell'intervallo che può essere rappresentato dall'intervallo UTC utilizzato dal server.  
...
-   **13**: Se si verifica il troncamento con la perdita di dati, viene generato un record di diagnostica con SQLSTATE 22001 e il messaggio "Dati stringa troncati a destra".  
  
     Il numero di cifre dei secondi frazionari (scala) viene determinato dalle dimensioni della colonna di destinazione in base alla tabella seguente:  
...
-   **N/D**: [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] Il comportamento esistente e quello precedente vengono mantenuti.  
