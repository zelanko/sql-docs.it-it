...
## <a name="opening-and-closing-a-database-connection"></a>Apertura e chiusura di una connessione al database  
 Quando si [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] utilizza il provider OLE DB Native Client, le applicazioni progettate per inizializzare un oggetto origine dati in modo asincrono possono impostare il DBPROPVAL_ASYNCH_INITIALIZE bit nella proprietà DBPROP_INIT_ASYNCH prima di chiamare **IDBInitialize::Initialize**. Quando questa proprietà è impostata, il provider restituisce immediatamente dalla chiamata a **Initialize** S_OK, se l'operazione è stata completata immediatamente, o DB_S_ASYNCHRONOUS, se l'inizializzazione continua in modo asincrono. Le applicazioni possono eseguire una query per l'interfaccia **IDBAsynchStatus** o [ISSAsynchStatus](../../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-ole-db.md)nell'oggetto origine dati e quindi chiamare **IDBAsynchStatus::GetStatus** o[ISSAsynchStatus::WaitForAsynchCompletion](../../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-waitforasynchcompletion-ole-db.md) per ottenere lo stato dell'inizializzazione.  
...
> [!NOTE]  
>  Gli oggetti origine dati utilizzati per il pool di [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] connessioni non possono chiamare l'interfaccia **ISSAsynchStatus** nel provider OLE DB native Client. L'interfaccia **ISSAsynchStatus** non è esposta per gli oggetti di origine dati inseriti in pool.  
>   
...
>   
>  Inoltre, la dll proxy/stub dei servizi remoti (in MDAC 2.8) [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] non può chiamare l'interfaccia **ISSAsynchStatus** in Native Client. L'interfaccia **ISSAsynchStatus** non è esposta attraverso i servizi remoti.  
>   
...
## <a name="see-also"></a>Vedere anche  
 ...
 [Proprietà e comportamenti del set di righeRowset Properties and Behaviors](../../../relational-databases/native-client-ole-db-rowsets/rowset-properties-and-behaviors.md)   
