...
|EventSubClass|**nvarchar**|Tipo di sottoclasse di evento in cui sono disponibili informazioni aggiuntive su ogni classe di evento. Questa colonna può contenere i valori seguenti:<br /><br /> **Table created**: indica che nel database è stata creata una tabella di parametri.<br /><br /> **Table drop attempt**: indica che il database ha provato a eliminare automaticamente una tabella di parametri inutilizzata per liberare risorse.<br /><br /> **Table drop attempt failed**: indica che il database ha provato a eliminare una tabella di parametri inutilizzata e che l'operazione ha avuto esito negativo. In [!INCLUDE[ssDE](../../includes/ssde-md.md)] verrà automaticamente ripianificata l'eliminazione della tabella di parametri per liberare risorse.<br /><br /> **Table dropped**: indica che il database ha eliminato correttamente una tabella di parametri.<br /><br /> **Table pinned**: indica che la tabella di parametri è stata contrassegnata con pinning per l'uso corrente tramite elaborazione interna.<br /><br /> **Table unpinned**: indica che per la tabella di parametri è stato annullato il pinning. L'elaborazione interna non richiede più l'utilizzo della tabella.<br /><br /> **Number of users incremented**: indica che il numero di sottoscrizioni di notifica delle query che fanno riferimento a una tabella di parametri è aumentato.<br /><br /> **Number of users decremented**: indica che il numero di sottoscrizioni di notifica delle query che fanno riferimento a una tabella di parametri è diminuito.<br /><br /> **LRU counter reset**: indica che il conteggio di utilizzi per la tabella di parametri è stato reimpostato.<br /><br /> **Cleanup task started**: indica quando ha avuto inizio la pulizia per tutte le sottoscrizioni nella tabella di parametri corrente. In genere, questa operazione ha inizio all'avvio del database o all'eliminazione di una tabella sottostante le sottoscrizioni di questa tabella di parametri.<br /><br /> **Cleanup task finished**: indica quando termina la pulizia per tutte le sottoscrizioni nella tabella di parametri corrente.|21|Sì|  
