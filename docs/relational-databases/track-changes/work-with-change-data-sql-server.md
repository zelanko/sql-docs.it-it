...
### <a name="querying-using-datetime-wrapper-functions"></a>Esecuzione di query utilizzando le funzioni del wrapper datetime  
 ...
 Le funzioni wrapper generate supportano la convenzione seguente per ignorare in maniera sistematica la cronologia di Change Data Capture: è previsto che il parametro @end_time dell'intervallo precedente sia utilizzato come parametro @start_time dell'intervallo successivo. La funzione wrapper esegue il mapping dei valori datetime ai valori LSN e si assicura che nessun dato venga perso o ripetuto se viene seguita questa convenzione.  
