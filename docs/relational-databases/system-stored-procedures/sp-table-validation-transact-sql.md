...
`[ @table_name = ] table_name` è il nome della tabella della vista utilizzata per i messaggi di output. *table_name* è di **tipo sysname**e il valore predefinito è **\@tabella**.  
 ...
 **sp_table_validation** viene utilizzato in tutti i tipi di replica. **sp_table_validation** non è supportato per i Publisher Oracle.  
 ...
 È probabile che i valori a virgola mobile generino differenze di checksum se è stata utilizzata l' **utilità bcp** in modalità carattere, che è il caso in cui la pubblicazione dispone di Sottoscrittori non [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Ciò è dovuto a differenze di precisione minime ma inevitabili nella conversione da e verso la modalità carattere.  
