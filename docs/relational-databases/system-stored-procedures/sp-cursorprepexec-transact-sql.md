 ...
 *handle preparato*  
 Identificatore dell' *handle* preparato generato.[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] *handle preparato* obbligatorio e restituisce **int**.  
...
> [!NOTE]  
>  Usare una stringa **ntext** come valore di input quando *stmt* è con parametri e il valore *scrollopt* PARAMETERIZED_STMT è on.  
...
> [!NOTE]  
>  Le regole per la specifica del valore stmt sono le stesse di quelle per sp_cursoropen, con l'eccezione che il tipo di dati della stringa *stmt* deve essere **ntext**.  
...
|--------------------|---------------------|  
|Quando AUTO_FETCH viene specificato con FAST_FORWARD cursori *RowCount* rappresenta il numero di righe da inserire nel buffer di recupero.|Rappresenta il numero di righe nel set di risultati. Quando viene specificato il valore AUTO_FETCH di *scrollopt* , *RowCount* restituisce il numero di righe recuperate nel buffer di recupero.|  
