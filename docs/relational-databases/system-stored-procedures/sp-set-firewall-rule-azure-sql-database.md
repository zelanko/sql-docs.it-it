...
## <a name="remarks"></a>Osservazioni  
 ...
 Quando si aggiunge un'impostazione del firewall a livello di server in cui gli indirizzi IP iniziale e finale sono uguali a `0.0.0.0`, si Abilita l'accesso al server [!INCLUDE[ssSDS](../../includes/sssds-md.md)] da Azure. Fornire un valore al parametro *Name* che consenta di ricordare l'impostazione del firewall a livello di server.  
...
## <a name="examples"></a>Esempi  
 Il codice seguente crea un'impostazione del firewall a livello di server denominata `Allow Azure` che consente l'accesso da Azure. Eseguire il comando seguente nel database master virtuale.  
...
```  
 ...
 Il codice seguente consente di creare un'impostazione del firewall a livello di server denominata `Example setting 1` solo per l'indirizzo IP `0.0.0.2`. Quindi, il `sp_set_firewall_rule` stored procedure viene richiamato per aggiornare l'indirizzo IP finale a `0.0.0.4`, in tale impostazione del firewall. Viene creato un intervallo che consente agli indirizzi IP `0.0.0.2`, `0.0.0.3`e `0.0.0.4` di accedere al server.  
