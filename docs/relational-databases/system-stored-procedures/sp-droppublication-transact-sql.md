...
`[ @publication = ] 'publication'` È il nome della pubblicazione da eliminare. *pubblicazione* viene **sysname**, non prevede alcun valore predefinito. Se **tutti** viene specificato, tutte le pubblicazioni vengono eliminate dal database di pubblicazione, ad eccezione di quelli con le sottoscrizioni.  
