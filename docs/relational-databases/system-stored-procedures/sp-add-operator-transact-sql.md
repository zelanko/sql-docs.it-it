...
`[ @name = ] 'name'` il nome di un operatore (destinatario notifiche). Questo nome deve essere univoco e non può contenere il carattere di percentuale ( **%** ). *Name* è di **tipo sysname**e non prevede alcun valore predefinito.  
...
`[ @email_address = ] 'email_address'` l'indirizzo di posta elettronica dell'operatore. Questa stringa viene passata direttamente al sistema di posta elettronica. *email_address* è di **tipo nvarchar (100)** e il valore predefinito è null.  
...
`[ @pager_address = ] 'pager_address'` l'indirizzo del cercapersone dell'operatore. Questa stringa viene passata direttamente al sistema di posta elettronica. *pager_address* è di **tipo nvarchar (100)** e il valore predefinito è null.  
...
`[ @saturday_pager_start_time = ] saturday_pager_start_time` tempo trascorso il quale il servizio **SQLServerAgent** invia una notifica tramite cercapersone all'operatore specificato il sabato. *saturday_pager_start_time* è di **tipo int**e il valore predefinito è 090000, che indica le ore 9:00 nel formato a 24 ore e deve essere immesso nel formato HHMMSS.  
...
`[ @netsend_address = ] 'netsend_address'` l'indirizzo di rete dell'operatore a cui viene inviato il messaggio di rete. *netsend_address*è di **tipo nvarchar (100)** e il valore predefinito è null.  
  
`[ @category_name = ] 'category'` il nome della categoria per questo operatore. *Category* è di **tipo sysname**e il valore predefinito è null.  
