...
`[ @schedule_name = ] 'schedule_name'` il nome della pianificazione. *schedule_name* è di **tipo sysname**e non prevede alcun valore predefinito.  
...
`[ @freq_type = ] freq_type` un valore che indica quando deve essere eseguito un processo. *freq_type* è di **tipo int**e il valore predefinito è **0**. i possibili valori sono i seguenti.  
...
`[ @freq_interval = ] freq_interval` i giorni in cui viene eseguito un processo. *freq_interval* è di **tipo int**e il valore predefinito è **1**e dipende dal valore di *freq_type*.  
  
|Value of *freq_type*|Effetti sui *freq_interval*|  
|---------------------------|--------------------------------|  
|**1** (once)|*freq_interval* non è utilizzato.|  
...
`[ @freq_subday_interval = ] freq_subday_interval` il numero di periodi di *freq_subday_type* tra ogni esecuzione di un processo. *freq_subday_interval* è di **tipo int**e il valore predefinito è **0**. Nota: l'intervallo dovrebbe superare i 10 secondi. *freq_subday_interval* viene ignorato nei casi in cui *freq_subday_type* è uguale a **1**.  
  
`[ @freq_relative_interval = ] freq_relative_interval` l'occorrenza di *freq_interval* di un processo ogni mese, se *freq_interval* è 32 (mensile relativo). *freq_relative_interval* è di **tipo int**e il valore predefinito è **0**. i possibili valori sono i seguenti. *freq_relative_interval* viene ignorato nei casi in cui *freq_type* non è uguale a 32.  
...
`[ @freq_recurrence_factor = ] freq_recurrence_factor` il numero di settimane o mesi che intercorre tra l'esecuzione pianificata di un processo. *freq_recurrence_factor* viene utilizzato solo se *freq_type* è **8**, **16**o **32**. *freq_recurrence_factor* è di **tipo int**e il valore predefinito è **0**.  
  
`[ @active_start_date = ] active_start_date` la data in cui può iniziare l'esecuzione di un processo. *active_start_date* è di **tipo int**e il valore predefinito è null, che indica la data odierna. La data è nel formato AAAAMMGG. Se *active_start_date* non è null, la data deve essere maggiore o uguale a 19900101.  
...
`[ @active_end_date = ] active_end_date` la data in cui l'esecuzione di un processo può essere arrestata. *active_end_date* è di **tipo int**e il valore predefinito è **99991231**, che indica il 31 dicembre 9999. La data è nel formato AAAAMMGG.  
  
`[ @active_start_time = ] active_start_time` l'ora di ogni giorno tra *active_start_date* e *active_end_date* per iniziare l'esecuzione di un processo. *active_start_time* è di **tipo int**e il valore predefinito è **000000**, che indica 12:00:00 A.M. nel formato a 24 ore e deve essere immesso nel formato HHMMSS.  
  
`[ @active_end_time = ] active_end_time` l'ora di ogni giorno tra *active_start_date* e *active_end_date* per terminare l'esecuzione di un processo. *active_end_time* è di **tipo int**e il valore predefinito è **235959**, che indica le 11:59:59. nel formato a 24 ore e deve essere immesso nel formato HHMMSS.  
  
`[ @owner_login_name = ] 'owner_login_name'` il nome dell'entità server proprietaria della pianificazione. *owner_login_name* è di **tipo sysname**e il valore predefinito è null, che indica che la pianificazione è di proprietà dell'autore.  
  
`[ @schedule_uid = ] _schedule_uidOUTPUT` un identificatore univoco per la pianificazione. *schedule_uid* è una variabile di tipo **uniqueidentifier**.  
