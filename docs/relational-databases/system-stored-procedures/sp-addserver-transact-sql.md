...
`[ @local = ] 'LOCAL'`Specifica che il server viene aggiunto come server locale. local è di tipo **varchar (10)** e il valore predefinito è null. ** \@** Se si specifica ** \@local** come **local** , il ** \@server** viene definito come nome del server locale e@SERVERNAME la funzione @ restituisce il valore del *Server*.  
