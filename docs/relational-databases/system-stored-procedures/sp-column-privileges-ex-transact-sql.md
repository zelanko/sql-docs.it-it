...
`[ @table_name = ] 'table_name'` È il nome della tabella che contiene la colonna specificata. *TABLE_NAME* viene **sysname**, con un valore predefinito è NULL.  
...
`[ @column_name = ] 'column_name'` È il nome della colonna per cui si desidera ottenere informazioni sui privilegi. *column_name* viene **sysname**, con un valore predefinito è NULL (tutte le comuni).  
