...
|sid|**binario (86)**|Identificatore univoco globale (GUID, Globally Unique Identifier) per l'utente.|  
...
|context_info|**binario (128)**|Dati archiviati in un batch tramite l'istruzione SET CONTEXT_INFO.|  
|sql_handle|**binario (20)**|Rappresenta il batch o l'oggetto attualmente in esecuzione.<br /><br /> **Nota** Questo valore è derivato dal batch o dall'indirizzo di memoria dell'oggetto. e non viene calcolato tramite l'algoritmo basato su hash di [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].|  
