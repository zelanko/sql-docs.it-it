...
1. Selezionare il **database** nell'elenco a discesa. 
1. Nell'elenco a discesa **Tipo backup** selezionare **Completo**. 
1. In **Componente di cui eseguire il backup** selezionare **Database**. 
1. Nella sezione **Destinazione** usare l'elenco a discesa **Backup su** per selezionare la destinazione di backup. Fare clic su **Aggiungi** per aggiungere ulteriori oggetti e/o destinazioni di backup. Per rimuovere una destinazione di backup, selezionarla e fare clic su **Rimuovi**. Per visualizzare il contenuto di una destinazione di backup, selezionarla e fare clic su **Contenuto**.
1. (Facoltativo) Rivedere le altre impostazioni disponibili nelle pagine **Opzioni supporti** e **Opzioni di backup**. Per altre informazioni sulle varie opzioni di backup, vedere le pagine [Generale](back-up-database-general-page.md), [Opzioni supporti](back-up-database-media-options-page.md) e [Opzioni di backup](back-up-database-backup-options-page.md). 



### <a name="additional-information"></a>Informazioni aggiuntive
- Dopo aver eseguito un backup completo, è possibile creare un backup differenziale del database. Per altre informazioni, vedere [Creare un backup differenziale del database &#40;SQL Server&#41;](../../relational-databases/backup-restore/create-a-differential-database-backup-sql-server.md).  
...
- L'opzione **Sovrascrivi supporti** può essere disabilitata nella pagina **Opzioni supporti** se si esegue il backup nell'URL. 
...
In questo esempio verrà eseguito il backup su disco del database `SQLTestDB` nel percorso di backup predefinito.  Il backup di `SQLTestDB` non è mai stato eseguito.
...
In questo esempio verrà eseguito il backup su disco del database `SQLTestDB` in `F:\MSSQL\BAK`.  In precedenza sono stati eseguiti backup di `SQLTestDB` .
1.  In **Esplora oggetti**connettersi a un'istanza del motore di database di SQL Server e, successivamente, espanderla.
...
6.  Immettere `F:\MSSQL\BAK\Sales_20160801.bak` nella casella di testo del **nome file** .
7.  Fare clic su **OK**.
8.  Fare clic su **OK**.
...
1.  In **Esplora oggetti**connettersi a un'istanza del motore di database di SQL Server e, successivamente, espanderla.
1. Aprire una finestra **Nuova query** ed eseguire i comandi seguenti per creare una [**chiave master del database**](../../relational-databases/security/encryption/create-a-database-master-key.md) e un [**certificato**](../../t-sql/statements/create-certificate-transact-sql.md) nel database `SQLTestDB`. 
...
1.  In **Esplora oggetti** espandere **Database**, fare clic con il pulsante destro del mouse su `SQLTestDB`, selezionare **Attività**, quindi fare clic su **Backup...** .
...
#### <a name="striped-backup-to-url-and-a-sql-server-credential-already-exists"></a>Il backup con striping nell'URL e una credenziale di SQL Server esistono già
...
#### <a name="a-shared-access-signature-exists-and-a-sql-server-credential-does-not-exist"></a>Esiste una firma di accesso condiviso e le credenziali di SQL Server non esistono
...
#### <a name="a-shared-access-signature-does-not-exist"></a>Non esiste una firma di accesso condiviso
