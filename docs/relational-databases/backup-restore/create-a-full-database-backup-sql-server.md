...
3. Fare clic con il pulsante destro del mouse sul database di cui eseguire il backup, scegliere **Attività** e quindi fare clic su **Backup**.
...
5. Nell'elenco a discesa **Tipo di backup** selezionare il tipo di backup desiderato. Il valore predefinito è **Completo**.
...
7. Nella sezione **Destinazione** esaminare il percorso predefinito per il file di backup (nella cartella ../mssql/data). Per eseguire il backup in un dispositivo diverso, modificare la selezione usando l'elenco a discesa **Backup su**. Per eseguire lo striping del set di backup tra più file per aumentare la velocità di backup, fare clic su **Aggiungi** per aggiungere altri oggetti e/o destinazioni di backup. Per rimuovere una destinazione di backup, selezionarla e fare clic su **Rimuovi**. Per visualizzare il contenuto di una destinazione di backup, selezionarla e fare clic su **Contenuto**.
8. (Facoltativo) Rivedere le altre impostazioni disponibili nelle pagine **Opzioni supporti** e **Opzioni di backup**. Per altre informazioni sulle varie opzioni di backup, vedere le pagine [Generale](back-up-database-general-page.md), [Opzioni supporti](back-up-database-media-options-page.md) e [Opzioni di backup](back-up-database-backup-options-page.md).
9. Fare clic su **OK** per avviare il backup.
10. Quando il backup viene completato correttamente, fare clic su **OK** per chiudere la finestra di dialogo Microsoft SQL Server Management Studio.
...
1. Dopo essersi connessi all'istanza appropriata del [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] [!INCLUDE[msCoName](../../includes/msconame-md.md)], in **Esplora oggetti** espandere l'albero del server.
2. Espandere i **database**, fare clic con il pulsante destro del mouse su `SQLTestDB`, scegliere **Attività**, quindi fare clic su **Backup...** .
3. Fare clic su **OK**.
4. Quando il backup viene completato correttamente, fare clic su **OK** per chiudere la finestra di dialogo Microsoft SQL Server Management Studio.
...
7. Fare clic su **OK** e quindi fare di nuovo clic su **OK** per avviare il backup.
8. Quando il backup viene completato correttamente, fare clic su **OK** per chiudere la finestra di dialogo Microsoft SQL Server Management Studio.
...
3. Eseguire i comandi seguenti per creare una [**chiave master del database**](../../relational-databases/security/encryption/create-a-database-master-key.md) e un [**certificato**](../../t-sql/statements/create-certificate-transact-sql.md) nel database `master`.  
...
4. In **Esplora oggetti**, nel nodo **Database**, fare clic con il pulsante destro del mouse su `SQLTestDB`, scegliere **Attività** e quindi fare clic su **Backup**.
...
5. Se in precedenza è stato registrato il contenitore di archiviazione di Azure che si vuole usare con SQL Server Management Studio, selezionarlo.
6. In caso contrario, fare clic su **Nuovo contenitore** per registrare un nuovo contenitore.
7. Nella finestra di dialogo **Connetti a una sottoscrizione Microsoft** accedere al proprio account.
8. Nella casella di testo dell'elenco a discesa **Seleziona account di archiviazione** selezionare l'account di archiviazione.
9. Nella casella di testo dell'elenco a discesa **Seleziona contenitore BLOB** selezionare il contenitore BLOB.
10. Nella casella del calendario dell'elenco a discesa **Scadenza criteri di accesso condiviso** selezionare una data di scadenza per i criteri di accesso condiviso creati in questo esempio.
11. Fare clic su **Crea credenziali** per generare una firma di accesso condiviso e le credenziali in SQL Server Management Studio.
12. Fare clic su **OK** per chiudere la finestra di dialogo **Connetti a una sottoscrizione Microsoft**.
13. Nella casella di testo **File di backup** modificare il nome del file di backup (facoltativo).
14. Fare clic su **OK** per chiudere la finestra di dialogo **Seleziona la destinazione di backup**.
15. Fare clic su **OK** per avviare il backup.
16. Quando il backup viene completato correttamente, fare clic su **OK** per chiudere la finestra di dialogo Microsoft SQL Server Management Studio.
...
## <a name="TsqlExample"></a> Esempi di Transact-SQL
...
L'esempio seguente consente di creare un backup di database completo del database di `SQLTestDB` nel percorso di backup predefinito dell'istanza del server `Computer\Instance`. Facoltativamente, questo esempio specifica **-BackupAction Database**. Per la sintassi completa e altri esempi, vedere [Backup-SqlDatabase](https://docs.microsoft.com/powershell/module/sqlserver/backup-sqldatabase).

> [!NOTE]
> Questi esempi richiedono il modulo SqlServer. Per determinare se è installato, eseguire `Get-Module -Name SqlServer`. Per installare questo modulo, eseguire `Install-Module -Name SqlServer` in una sessione di amministrazione di PowerShell. Per altre informazioni, vedere [Provider PowerShell per SQL Server](https://docs.microsoft.com/sql/powershell/sql-server-powershell-provider).
