...
-   Se lo schema non specifica colonne di overflow tramite l'annotazione **SQL: overflow-field** , il caricamento bulk XML ignora tutti i dati presenti nel documento XML ma non è descritto nello schema di mapping.  
    ...
    ```  
     ...
     Il caricamento bulk XML ignora questi ** \<elementi dell'ordine>** . Tuttavia, se si utilizza l'annotazione **SQL: overflow-field**nello schema per identificare una colonna come colonna di overflow, il caricamento bulk XML archivia tutti i dati non utilizzati in questa colonna.  
