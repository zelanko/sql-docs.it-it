...
description: Informazioni sulle linee guida e sulle limitazioni dell'utilizzo del caricamento bulk XML in SQLXML 4,0.
ms.date: 03/16/2017
ms.prod: sql
...
-   Se lo schema non specifica colonne di overflow tramite l'annotazione **SQL: overflow-field** , il caricamento bulk XML ignora tutti i dati presenti nel documento XML ma non è descritto nello schema di mapping.  
    ...
    ```  
     ...
     Questi elementi vengono ignorati dal caricamento bulk XML **\<Order>** . Tuttavia, se si utilizza l'annotazione **SQL: overflow-field**nello schema per identificare una colonna come colonna di overflow, il caricamento bulk XML archivia tutti i dati non utilizzati in questa colonna.  
...
-   In presenza di più di una relazione tra tabelle, SchemaGen tenta di creare una singola relazione che include tutte le chiavi incluse tra le due tabelle. Questa limitazione può essere la causa di un errore [!INCLUDE[tsql](../../../includes/tsql-md.md)].  
...
-   Alcuni flag della proprietà non sono compatibili con altri flag della proprietà stessa. Il caricamento bulk, ad esempio, non supporta **IgnoreDuplicateKeys = true** insieme a **KeepIdentity = false**. Quando **KeepIdentity = false**, il caricamento bulk prevede che il server generi i valori della chiave. Le tabelle devono avere un vincolo **Identity** per la chiave. Il server non genererà chiavi duplicate, ovvero non è necessario che **IgnoreDuplicateKeys** sia impostato su **true**. **IgnoreDuplicateKeys** deve essere impostato su **true** solo quando si caricano i valori di chiave primaria dai dati in entrata in una tabella con righe ed è possibile che si verifichi un conflitto di valori di chiave primaria.  
