 ...
 **SQLGetInfo** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] in Native Client differisce [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] da **SQLGetInfo** nel driver ODBC (SQLSRV32. DLL) quando **SQLGetInfo** viene chiamato con SQL_KEYWORDS e una lunghezza del buffer 0.  Tramite il driver [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client viene restituito SQL_SUCCESS, mentre tramite il driver ODBC [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] viene restituito SQL_SUCCESS_WITH_INFO.  Tuttavia, quando viene chiamato con una lunghezza del buffer diversa da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] zero inferiore alla stringa della parola chiave di output, SQLGetInfo in Native Client restituisce SQL_SUCCESS_WITH_INFO e un SQLState pari a 01004.However, when called with a diversazero buffer length that is less than the output keyword string, **SQLGetInfo** in Native Client returns SQL_SUCCESS_WITH_INFO and a SQLState of 01004.  
  
|fInfoType|rgbInfoValue|  
|---------------|------------------|  
|SQL_ACCESSIBLE_PROCEDURES|"Y"|  
|SQL_ACCESSIBLE_TABLES|"Y"|  
|SQL_ACTIVE_CONNECTIONS|Il numero di connessioni è limitato da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Il driver restituisce 0 per questa richiesta **SQLGetInfo.The** driver returns 0 for this SQLGetInfo request.|  
|SQL_ACTIVE_ENVIRONMENTS|Il numero di ambienti non è limitato dal driver. Il driver restituisce 0 per questa richiesta **SQLGetInfo.The** driver returns 0 for this SQLGetInfo request.|  
|SQL_ACTIVE_STATEMENTS|Il driver restituisce 1 per questa richiesta **SQLGetInfo.The** driver returns 1 for this SQLGetInfo request. Il numero di handle di istruzione disponibili in un'applicazione non è limitato dal driver, ma l'esecuzione predefinita in un handle di istruzione bloccherà l'esecuzione in tutti gli altri handle.|  
...
|SQL_CONVERT_DATE|Nessun supporto per la conversione del tipo di dati ODBC SQL_TYPE_DATE. Il [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] driver ODBC Native [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Client supporta il tipo di dati **datetime** come tipo ODBC SQL_TYPE_TIMESTAMP. Vedere SQL_CONVERT_TIMESTAMP riportato di seguito.|  
...
|SQL_CONVERT_DOUBLE|Nessun supporto per la conversione del tipo di dati ODBC SQL_DOUBLE. Il [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] driver ODBC Native Client supporta il tipo di dati SQL_DOUBLE ODBC come SQL_FLOAT. Vedere SQL_CONVERT_FLOAT riportato di seguito.|  
...
|SQL_DATA_SOURCE_NAME|Nome origine dati corrente. Imposta il valore a cui *punta StringLengthPtr* su 0 se la connessione non specifica un nome di origine dati.|  
...
## <a name="see-also"></a>Vedere anche  
 [Funzione SQLGetInfoSQLGetInfo Function](https://go.microsoft.com/fwlink/?LinkId=59354)   
