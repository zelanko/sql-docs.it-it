...
```  
 ...
 Per comprendere **NON VISUAL,** nell'esempio seguente è una query di [Adventure Works] per ottenere le cifre di [Reseller Sales Amount] in una tabella in cui sono le categorie di prodotti e i tipi business dei rivenditori sono le righe. I totali vengono forniti per sia i prodotti che per i rivenditori.  
...
|Warehouse|$38,726,913.48|$331,169.64|$932,521.23|  
 ...
 Nell'esempio seguente viene dimostrato come si utilizzano i membri calcolati nelle sub-SELECT per filtrarli. Per essere in grado di riprodurre questo esempio, la connessione deve essere stabilita utilizzando il parametro di stringa di connessione *sottoquery = 1*.  
