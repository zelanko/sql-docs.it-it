...
> [!NOTE]  
>  Questa istruzione è stata inclusa per compatibilità con le versioni precedenti. Le azioni nuove [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]a, ad esempio le azioni drill-through o report, non sono supportate.  
  
## <a name="action-types"></a>Tipi di azioni  
 Nella tabella seguente vengono descritti i diversi tipi di azioni disponibili [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]in.  
  
|Tipo di azione|Descrizione|  
|-----------------|-----------------|  
|**URL**|Viene restituita una stringa costituita da un URL a cui accedere tramite browser Internet.<br /><br /> Nota: se questa azione non inizia `https://` con o `https://`, l'azione non sarà disponibile per il browser, a meno che **SafetyOptions** non sia impostato su **DBPROPVAL_MSMD_SAFETY_OPTIONS_ALLOW_ALL**.|  
