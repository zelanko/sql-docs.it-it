...
ms.locfileid: "76762255"
---
# <a name="o2ss0331-unable-to-convert-fetch-statement-error"></a>O2SS0331: non è possibile convertire l'istruzione FETCH (Error)

Questo articolo descrive il motivo per cui SQL Server Migration Assistant (SSMA) per Oracle genera un errore `FETCH ... INTO` durante la conversione dell'istruzione in `SYS_REFCURSOR` una procedura in cui un oggetto viene restituito come parametro.

## <a name="background"></a>Informazioni

In Oracle, `SYS_REFCURSOR` viene utilizzato per passare i cursori da e a un stored procedure.

Ogni volta che SSMA rileva una variabile per `SYS_REFCURSOR` come tipo restituito della routine e utilizza la stessa variabile con `FETCH ... INTO` l'istruzione, viene generato un messaggio di errore.
