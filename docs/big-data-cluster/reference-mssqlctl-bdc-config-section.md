---
title: riferimento a config mssqlctl integrazione applicativa dei dati
titleSuffix: SQL Server big data clusters
description: Articolo di riferimento per i comandi sezione di configurazione di mssqlctl integrazione applicativa dei dati.
author: rothja
ms.author: jroth
manager: jroth
ms.date: 06/26/2019
...
L'articolo seguente fornisce informazioni di riferimento per la **sezione di configurazione di integrazione applicativa dei dati** comandi nel **mssqlctl** dello strumento. Per altre informazioni sulle altre **mssqlctl** comandi, vedere [mssqlctl riferimento](reference-mssqlctl.md).

## <a name="commands"></a>Comandi
|     |     |
| --- | --- |
[show sezione Configurazione di mssqlctl integrazione applicativa dei dati](#mssqlctl-bdc-config-section-show) | Ottiene una sezione da un profilo di configurazione.
[gruppo di sezione di configurazione di integrazione applicativa dei dati mssqlctl](#mssqlctl-bdc-config-section-set) | Imposta una sezione per un profilo di configurazione.
## <a name="mssqlctl-bdc-config-section-show"></a>show sezione Configurazione di mssqlctl integrazione applicativa dei dati
Ottiene la sezione specificata dal profilo di configurazione selezionato in base al percorso json specificato.
```bash
...
#### `--json-path -j`
Il percorso json che comporta la sezione o il valore desiderato dal file JSON del profilo di configurazione, vale a dire key1.key2.key3. Usa il linguaggio di query jsonpath https://github.com/h2non/jsonpath-ng, ad esempio: -j ' $. spec.pools [? ( @.spec.type = = "Master")]... degli endpoint
#### `--config-profile -c`
Percorso del profilo di configurazione di integrazione applicativa dei dati.
...
### <a name="required-parameters"></a>Parametri obbligatori
#### `--config-profile -c`
Percorso del profilo di configurazione di integrazione applicativa dei dati del file di configurazione da impostare
### <a name="optional-parameters"></a>Parametri facoltativi
#### `--json-values -j`
