---
title: riferimento di stato cluster mssqlctl
titleSuffix: SQL Server big data clusters
description: Articolo di riferimento per i comandi di stato cluster mssqlctl.
author: rothja
ms.author: jroth
manager: jroth
ms.date: 05/22/2019
...
ms.locfileid: "66779355"
---
# <a name="mssqlctl-cluster-status"></a>mssqlctl cluster status

[!INCLUDE[tsql-appliesto-ssver15-xxxx-xxxx-xxx](../includes/tsql-appliesto-ssver15-xxxx-xxxx-xxx.md)]

L'articolo seguente fornisce informazioni di riferimento per la **stato del cluster** comandi nel **mssqlctl** dello strumento. Per altre informazioni sulle altre **mssqlctl** comandi, vedere [mssqlctl riferimento](reference-mssqlctl.md).

## <a name="commands"></a>Comandi
|     |     |
| --- | --- |
[Mostra lo stato di mssqlctl cluster](#mssqlctl-cluster-status-show) | Mostra lo stato del cluster.
## <a name="mssqlctl-cluster-status-show"></a>Mostra lo stato di mssqlctl cluster
Mostra lo stato del cluster.
```bash
mssqlctl cluster status show 
```
### <a name="examples"></a>Esempi
Stato del cluster in cui l'utente è connesso.
```bash
mssqlctl cluster status show
```
