...
L'API Valutazione SQL offre uno strumento per valutare la configurazione dell'istanza di SQL Server in uso per le procedure consigliate. L'API viene fornita con un set di regole contenente le regole per le procedure consigliate dal team di SQL Server. Questo set di regole è migliorato con il rilascio di nuove versioni ma, al tempo stesso, l'API è creata con la finalità di offrire una soluzione altamente personalizzabile ed estendibile. Gli utenti possono quindi ottimizzare le regole predefinite e crearne di proprie.
