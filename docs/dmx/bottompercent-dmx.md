...
## <a name="remarks"></a>Note  
 La funzione **BottomPercent** restituisce le righe più in basso in ordine crescente di rango. Il rango è basato sul valore valutato dell'espressione di \<rango > argomento per ogni riga, in modo che la somma \<dell'espressione di rango > valori sia almeno quella specificata dalla \<percentuale > argomento. **BottomPercent** restituisce il più piccolo numero di elementi possibile mentre soddisfa ancora il valore percentuale specificato.  
