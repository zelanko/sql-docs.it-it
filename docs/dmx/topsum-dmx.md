...
## <a name="remarks"></a>Note  
 La funzione TopSum restituisce le righe più in alto in ordine di rango decrescente in base al valore valutato dell'espressione \<di rango > argomento per ogni riga, in modo che \<la somma dei valori dell'espressione di rango > sia almeno quella specificata totale specificato dall' \<argomento Sum >. **TopSum** restituisce il più piccolo numero di elementi possibile mentre soddisfa ancora il valore della somma specificato.  
