...
-   SQL_PARAM_UNUSED: questo set di parametri è stato inutilizzato, probabilmente a causa del fatto che alcuni set di parametri precedenti hanno causato un errore che ha interrotto l'ulteriore elaborazione o perché SQL_PARAM_IGNORE è stato impostato per il set di parametri nella matrice specificata dalla SQL_DESC_ARRAY_ STATUS_PTR campo di APD.  
 ...
 **SQL_DESC_PRECISION [tutti]**  
 Questo campo di record SQLSMALLINT contiene il numero di cifre per un tipo numerico esatto, il numero di bit in mantissa (precisione binaria) per un tipo numerico approssimativo o il numero di cifre nel componente per i secondi frazionari per la SQL_TYPE_TIME, SQL_TYPE _TIMESTAMP o SQL_INTERVAL_SECOND tipo di dati. Questo campo non è definito per tutti gli altri tipi di dati.  
