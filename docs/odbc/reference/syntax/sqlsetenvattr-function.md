...
|HY010|Errore sequenza funzione|(DM) è stato allocato un handle di connessione in *EnvironmentHandle*.<br /><br /> (DM) **SQL_ATTR_ODBC_VERSION** non è stato impostato con **SQLSetEnvAttr** e l' *attributo* non è uguale a **SQL_ATTR_ODBC_VERSION**. Non è necessario impostare **SQL_ATTR_ODBC_VERSION** in modo esplicito se si usa **SQLAllocHandleStd**.|  
...
|HY090|Lunghezza della stringa o del buffer non valida|L'argomento *StringLength* è minore di 0 ma non è SQL_NTS.|  
...
|HYC00|Funzionalità facoltativa non implementata|Il valore specificato per l' *attributo* argument è un attributo dell'ambiente ODBC valido per la versione di ODBC supportata dal driver, ma non è supportato dal driver.<br /><br /> (DM) l'argomento dell' *attributo* è SQL_ATTR_OUTPUT_NTS e *ValuePtr* è SQL_FALSE.|  
