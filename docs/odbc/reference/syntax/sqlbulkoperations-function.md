...
## <a name="row-status-array"></a>Matrice di stato riga  
 La matrice di stato della riga contiene i valori di stato per ogni riga di dati nel set di righe dopo una chiamata a **SQLBulkOperations**. Il driver imposta i valori di stato in questa matrice dopo una chiamataa SQLFetch, **SQLFetchScroll**, **SQLSetPos**o **SQLBulkOperations**. Questa matrice viene inizialmente popolata da una chiamata a **SQLBulkOperations** se SQLFetch o **SQLFetchScroll** non è stato chiamato prima di **SQLBulkOperations**. A questa matrice viene fatto riferimento dall'attributo dell'istruzione SQL_ATTR_ROW_STATUS_PTR. Il numero di elementi nelle matrici di stato della riga deve essere uguale al numero di righe nel set di righe, come definito dall'attributo dell'istruzione SQL_ATTR_ROW_ARRAY_SIZE. Per informazioni su questa matrice di stato della riga , vedere [SQLFetch](../../../odbc/reference/syntax/sqlfetch-function.md).  
