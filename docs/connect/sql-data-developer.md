...
* [Motore di database di SQL Server](../database-engine/install-windows/install-sql-server-database-engine.md): usare il motore di database di SQL Server per creare database relazionali per i dati di elaborazione di transazioni online o i dati OLAP (Online Analytical Processing). 
* [SQL di Azure](/azure/azure-sql/azure-sql-iaas-vs-paas-what-is-overview): usare Azure SQL per trasferire il database nel cloud 
...
* [Local DB](../database-engine/configure-windows/sql-server-express-localdb.md): usare Local DB durante lo sviluppo per scrivere e testare il codice Transact-SQL senza dover gestire un'istanza server completa di SQL Server.
