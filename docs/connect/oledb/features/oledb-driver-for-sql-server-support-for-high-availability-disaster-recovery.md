...
Il driver OLE DB per SQL Server supporta le parole chiave **ApplicationIntent** e **MultiSubnetFailover** .   
  
Sono state aggiunte le due parole chiave della stringa di [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] connessione OLE DB per supportare il driver OLE DB per SQL Server:  
...
Un driver OLE DB per SQL Server applicazione può usare uno dei metodi per specificare la finalità dell'applicazione:  
...
Un driver OLE DB per SQL Server applicazione può utilizzare uno dei metodi seguenti per impostare l'opzione MultiSubnetFailover:  
...
-   **IDBProperties::SetProperties**  
Per impostare il valore della proprietà **MultiSubnetFailover** , chiamare **IDBProperties::** SetProperty passando la proprietà **SSPROP_INIT_MULTISUBNETFAILOVER** con il valore **VARIANT_TRUE** o **VARIANT_FALSE** o **DBPROP_INIT_ Proprietà PROVIDERSTRING** con valore contenente "**MultiSubnetFailover = Yes**" o "**MultiSubnetFailover = no**".
