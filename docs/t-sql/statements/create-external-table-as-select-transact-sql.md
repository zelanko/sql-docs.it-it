...
Per usare CREATE EXTERNAL TABLE AS SELECT con questi caratteri, è prima di tutto necessario applicare CREATE EXTERNAL TABLE AS SELECT ai dati per i file di testo delimitati in cui è quindi possibile eseguire la conversione in Parquet oppure ORC con uno strumento esterno.
