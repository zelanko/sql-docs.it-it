...
La disabilitazione di Stretch non comporta la rimozione del database remoto. Se si vuole eliminare il database remoto, è necessario eliminarlo tramite il portale di Azure.
...
### <a name="g-enabling-the-query-store-with-custom-capture-policy-options"></a>G. Abilitazione di Query Store con opzioni di criteri di acquisizione personalizzati
...
OFF         
Specifica che Query Optimizer non aggiorna le statistiche quando vengono usate da una query né quando potrebbero essere obsolete. L'impostazione di questa opzione su OFF può determinare piani di query e prestazioni di esecuzione delle query non ottimali.
...
AUTOMATIC_TUNING = { AUTO | INHERIT | CUSTOM }         
AUTO         
L'impostazione del valore di ottimizzazione automatica su AUTO fa in modo che all'ottimizzazione automatica vengano applicate le impostazioni predefinite di configurazione di Azure.
...
CUSTOM         
Se si utilizza il valore CUSTOM, è necessario personalizzare manualmente ciascuna delle opzioni di ottimizzazione automatica disponibili nei database.
...
MAX_STORAGE_SIZE_MB         
Determina lo spazio allocato per l'archivio query. MAX_STORAGE_SIZE_MB è di tipo **bigint**.
...
ALL         
Vengono acquisite tutte le query. Si tratta del valore di configurazione predefinito.

AUTO         
Acquisisce le query pertinenti in base al conteggio delle esecuzioni e al consumo delle risorse. Si tratta del valore di configurazione predefinito per [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]
...
ENCRYPTION { ON | OFF } ON Imposta il database da crittografare.
...
ON | OFF   
Controlla se l'archivio query è abilitato in questo data warehouse.     
...
### <a name="a-enabling-the-query-store"></a>A. Abilitazione dell'archivio di query
...
### <a name="b-enabling-the-query-store-with-wait-statistics"></a>B. Abilitazione di Query Store con statistiche di attesa
...
### <a name="c-enabling-the-query-store-with-custom-capture-policy-options"></a>C. Abilitazione di Query Store con opzioni di criteri di acquisizione personalizzati
...
### <a name="d-enable-result-set-caching-for-a-database"></a>D. Abilitare la memorizzazione nella cache dei set di risultati per un database
...
### <a name="d-disable-result-set-caching-for-a-database"></a>D. Disabilitare la memorizzazione nella cache dei set di risultati per un database
...
### <a name="d-check-for-number-of-queries-with-result-set-cache-hit-and-cache-miss"></a>D. Verificare il numero di query con riscontri e mancati riscontri nella cache dei set di risultati
...
### <a name="d-check-for-result-set-cache-hit-or-cache-miss-for-a-query"></a>D. Verificare i riscontri o i mancati riscontri nella cache dei set di risultati per una query
...
### <a name="d-check-for-all-queries-with-result-set-cache-hits"></a>D. Verificare tutte le query con riscontri nella cache dei set di risultati
